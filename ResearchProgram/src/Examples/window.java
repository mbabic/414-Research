...
class processor {
	...
	public processor() {
		...
		if (face_cascade0.empty() || face_cascade1.empty()
				|| face_cascade2.empty()) {
	...
	public Mat detect(Mat inputframe) {
		...
		for (Rect rect : faces1.toArray()) {
			Core.rectangle(mRgba,
					new Point(rect.x, rect.y - rect.height * 0.25), new Point(
							rect.x + rect.width, rect.y + rect.height),
					new Scalar(0, 255, 0), 2/* Core.FILLED */, 8, 0);
		...
		for (Rect rect : faces2.toArray()) {
			Core.rectangle(mRgba,
					new Point(rect.x, rect.y - rect.height * 0.25), new Point(
							rect.x + rect.width, rect.y + rect.height),
					new Scalar(0, 0, 255), 2/* Core.FILLED */, 8, 0);
