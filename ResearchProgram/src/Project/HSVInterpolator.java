...
/**
 * Performs interpolation operations in the HSV colour space. Implements the
...
public class HSVInterpolator implements Interpolator {
	...
	/**
	 * Perform interpolation use barycentric coordinates (triangles) in the HSV
	 * colour space.
	 * 
	 * @param v0
	...
	public CvScalar barycentricInterpolate(CvScalar v0, CvScalar v1,
		...
		// Interpolate
		h = (lambda0 * v0.val(0)) + (lambda1 * v1.val(0))
				+ (lambda2 * v2.val(0));
		s = (lambda0 * v0.val(1)) + (lambda1 * v1.val(1))
				+ (lambda2 * v2.val(1));
		v = (lambda0 * v0.val(2)) + (lambda1 * v1.val(2))
				+ (lambda2 * v2.val(2));
