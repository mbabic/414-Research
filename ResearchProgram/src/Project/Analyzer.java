...
public class Analyzer {
	 ...
	 * (bImage) storing the result in cImage in a naive way: Replace pixels
	 * falling along the rectangles in rects with the values of its neighbouring
	 * pixels (without interpolation, simple value replacement)
	 * 
	 * @param cImage
	 * @param bImage
	 * @param fImage
	 * @param rects
	 */
	public void naiveRecombination(IplImage cImage, IplImage bImage,
			IplImage fImage, ArrayList<CvRect> rects) {
		CvScalar b, f;
	...
	/**
	 * Combine the given foreground image (fImage) and background image (bImage)
	 * into cImage by taking the absolute difference between the images and
	 * interpolating RGB components linearly and separately along each line
	 * defined by the given rects.
	 * 
	 * @param cImage
	 * @param bImage
	 * @param fImage
	 * @param rects
	 */
	private void linearInterpolationRecombination(IplImage cImage,
				...
				if ((0 <= j && j <= imgHeight) && (2 <= x && x <= imgWidth - 2)) {
					...
					if (!(b.val(0) == 0 && b.val(1) == 0 && b.val(2) == 0)) {
						...
						cvSet2D(cImage, j, x + width + 1,
								interpolateRGB(b, f, 0.25));
					}
					if (!(f.val(0) == 0 && f.val(1) == 0 && f.val(2) == 0)) {
						cvSet2D(cImage, j, x + width - 1,
								interpolateRGB(b, f, 0.75));
					}
				}
			}
	...
	/**
	 * Combine the given foreground image (fImage) and background image (bImage)
	 * into cImage by taking the absolute difference between the images,
	 * transform the image to the HSV color space, interpolating along the lines
	 * defines by the rectangles in rects, and re-converting to the RGB color
	 * space.
	 * 
	 * @param cImage
	 * @param bImage
	 ...
	 * @param rects
	 */
	private void hsvInterpolationRecombination(IplImage cImage,
			IplImage bImage, IplImage fImage, ArrayList<CvRect> rects) {
			...
			for (int j = y; j < y + height; j++) {
				...
				if ((0 <= j && j <= imgHeight) && (2 <= x && x <= imgWidth - 2)) {
					...
					if (!(b.val(0) == 0)) {
						...
						cvSet2D(cHsv, j, x + width + 1,
								interpolateHSV(b, f, 0.25));
					}
					if (!(f.val(0) == 0)) {
						cvSet2D(cHsv, j, x + width - 1,
								interpolateHSV(b, f, 0.75));
					}
				}
			}
