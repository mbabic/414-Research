...
public class RectAnalyzer {
	 ...
	 * 
	 * @param rects
	 *            The list of rects for which the list of bounding rects is to
	 *            be determined.
	 * @return List of bounding rects.
	 */
	public static ArrayList<CvRect> getBoundingRects(ArrayList<CvRect> origRects) {
	...
	public static CvRect getMinBoundingRect(CvRect r1, CvRect r2) {
		...
		if ((y1 < y2) && ((y1 + h1) < (y2 + h2))) {
			...
			// r2 is above r1 and the height of r2 does not contain the height
			// of
			// r1
			ret.y(y2);
			ret.height(y1 - y2 + h1);
	 ...
	 * @param r2
	 *            The second rectangle
	 * @return Returns the min between topLeftDist and midpointDist.
	 */
	 ...
	 * @param r2
	 *            The second rectangle
	 * @return Returns the square of the distance between the top left corners
	 *         of the given CvRects.
	 */
	public static double topLeftDist(CvRect r1, CvRect r2) {
		return Math.pow(r1.x() - r2.x(), 2) + Math.pow(r1.y() - r2.y(), 2);
	}
	 ...
	 * @param r2
	 *            The second rectangle
	 * @return The square of the distance between the centers of the given
	 *         rectangles.
	 */
	public static double midpointDist(CvRect r1, CvRect r2) {
		return Math.pow(r1.x() + ((double) r1.width() / 2f) - r2.x()
				- ((double) r2.width() / 2f), 2)
				+ Math.pow(r1.y() + ((double) r1.height() / 2f) - r2.x()
						- ((double) r2.height() / 2f), 2);
	}
	 ...
	 *            rectangle is to be determined.
	 * @param rect
	 *            The rectangle for which we wish to determine if the given
	 *            point lies within.
	 */
